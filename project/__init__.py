...
def index():
    ...
    books=db.ExecuteQuery("""
                            ...
                            LEFT JOIN (select * from reviews where review_status=2) as reviews ON reviews.book = books.id
    ...
    flash(books_descriptions)
    if page>max_page:
        flash("Такой страницы не существует","danger")
        return redirect(url_for('index',page=1))
    else:
        return render_template("courselists.html", books=books, 
    ...
    if form.validate_on_submit():
        try:
            ...
            if file:
                ...
                flash('Cover image is missing.', 'danger')
            ...
            flash('Book was successfully created.', 'success')
            return redirect(url_for('index'))
        except Exception as e:
            flash(f"Error: {e}", 'danger')
            flash('Not created.', 'danger')
...
def show_book(book_id):
    ...
    return render_template('show.html',book=book,existing_review=existing_review, book_description=book_description)
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            try:
                ...
                flash(f"Error during database operation: {e}", "danger")



            # db.ExecuteQuery("DELETE from m2m_books_genres where book_id=%s",[book_id], query_type='post')
            # flash("Started editing")
            # for genre_id in form.genres.data:
            #     db.ExecuteQuery("INSERT INTO m2m_books_genres values(%s,%s)",[book_id, genre_id],query_type='post')
            # db.ExecuteQuery("UPDATE books set name=%s,description=%s,year=%s,publisher=%s,author=%s,pages=%s where id=%s",[form.title.data,
            #                                                                                                               form.description.data,
            #                                                                                                               form.year.data,
            #                                                                                                               form.publisher.data,form.author.data,
            #                                                                                                               form.pages.data,book_id], query_type='post')
