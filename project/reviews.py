...
def create_review(book_id):
    ...
    if request.method == 'POST':
        ...
        if result==False:
            ...
            flash('Review was successfully created.', 'success')
            return redirect(url_for('.index', book_id=book_id))
...
def allReviews():
    ...
    data = db.ExecuteQuery("select a.*, b.status, c.name from reviews a join reviews_status b on a.review_status = b.id join books c on a.book = c.id where a.review_status = 1 LIMIT %s OFFSET %s",[REQUESTS_PER_PAGE,((page-1)*REQUESTS_PER_PAGE)], query_type="get" )
...
def status(review_id):
    ...
    if res == "decline":
        ...
        if result == False:
            flash("Ошибка на стороне сервера", "danger")
            return redirect(url_for('reviews.allReviews',page=1))
    elif res == "accept":
